    ...
    fallbacks: store.accounts
      ...
      .map((account) => {
        ...
        return {
          ...
          accountId: (account as OAuthAccount).accountId,
          quota: fallbackQuota
            ? {
    ...
    auth: {
      ...
      async loader(getAuth) {
        ...
        function admissionQuotaDecision(
          ...
          now: number,
          fileMainAccountId?: string,
          currentMainAccountId?: string,
          ...
          const useFile =
            (fileMainAccountId === undefined ||
              fileMainAccountId === currentMainAccountId) &&
            fileQuota != null &&
        ...
        async function usableFallbackCandidates(
          ...
          for (const candidate of current) {
            const decision = admissionQuotaDecision(
              quotaManager.peekFallbackForPolicy(candidate.quotaAccountId),
              fileQuotas.get(candidate.quotaAccountId),
              now,
            )
